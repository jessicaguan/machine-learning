...
% yTest = y(testIdx);
% 
% % writetable(xTest, 'xTestRFCannabis.csv');
% % writecell(yTest, 'yTestRFCannabis.csv');
% 
...
% t = templateTree('Reproducible',true, 'MinLeafSize', 1, 'MaxNumSplits', 2102);
% 
% % create random forest model with fitcensemble and with the optimized
% % hyperparameters, train the model
% rf_mdl_c = fitcensemble(xTrain, yTrain, 'ClassNames', classNames,...
%     'Method', 'Bag', 'NumLearningCycles',387, 'Learners', t);
% 
% 
% y_pred_train = predict(rf_mdl_c, xTrain);
% 
...
% resubLossValue = resubLoss(rf_mdl_c);
% 
% % get accuracy 
...
% cvMdl = crossval(rf_mdl_c, 'KFold', 5); 
% 
% % get k fold loss
...
% figure;
% confusionchart(yTrain, predict(rf_mdl_c, xTrain), 'ColumnSummary', 'column-normalized', 'RowSummary', 'row-normalized');
...
xTest = readtable('xTestRFCannabis.csv');
yTest = readtable('yTestRFCannabis.csv');
yTest = table2cell(yTest);


rng('default')
y_pred_test = predict(rf_mdl_c, xTest);
...
% create cross validated model
cvMdlA = crossval(rf_mdl_c, 'KFold', 5); 
